...
const ProductFilters = ({ setNewProducts, filteredProducts }) => {
	...
	const handleSearchTerm = (e) => {
		setNewProducts(filteredProducts.filter((item) => item.name.includes(e.target.value)));
	};
			...
			<div className='hidden md:contents' id='filters'>
				<div>
					<div>
						...
						<ul className='text-base transition-all duration-300 ease-linear' id='category'>
							{categories.map((category, index) => (
								<li
									key={index}
									className={`mb-2 text-lg tracking-widest font-normal ${
										activeCategory === category ? 'bg-gray-300 rounded-lg p-1' : ''
									}`}
									onClick={(e) => {
