...
const Button = ({ title, onClick, py, px }) => {
	return (
		<button className={`px-${px} py-${py} relative rounded group font-medium text-white text-center inline-block`} onClick={onClick}>
			...
			<span className='relative text-center text-white text-lg'>{title}</span>
		</button>
	);
};
